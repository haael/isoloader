...
Error:
    if (TempPartitionSpecs) {
        LOG_DEBUG(L"EnumeratePartitions: freeing TempPartitionSpecs");
        UINTN j = 0;
...
Success:
    if (HandleBuffer) {
        LOG_DEBUG(L"EnumeratePartitions: freeing HandleBuffer");
        FreePool(HandleBuffer);
    }
    if (DevicePathStr) {
        LOG_DEBUG(L"EnumeratePartitions: freeing DevicePathStr");
        FreePool(DevicePathStr);
    }
